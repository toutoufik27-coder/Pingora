  ...
  return (
    <figure>
      <div className="flex gap-3" aria-hidden>
        <div className="flex h-44 flex-col justify-between py-0 text-right text-xs text-slate-400">
          {ticks.map((t) => (
        ...
        <div className="relative h-44 flex-1">
          {ticks.map((t, i) => (
            <div key={t} className="absolute inset-x-0 border-t border-slate-100" style={{ top: `${(i / (ticks.length - 1)) * 100}%` }} />
          ))}
            ...
            {points.map((p, i) => {
              ...
              return (
                <div key={p.month} className="group relative flex h-full w-full max-w-16 items-end justify-center">
                  <div
                    className="w-full max-w-6 rounded-t bg-[#0d9488] transition-opacity group-hover:opacity-80"
                    style={{ height: `${Math.max(height, p.cents > 0 ? 1.5 : 0)}%` }}
                  />
                  {i === last && p.cents > 0 ? (
                    <span className="tabular absolute text-xs font-semibold text-slate-900" style={{ bottom: `calc(${height}% + 4px)` }}>
                      {formatMoney(Math.round(p.cents / 100) * 100).replace(/\.00$/, "")}
                    </span>
                  ) : null}
                  <div className="pointer-events-none absolute bottom-full z-10 mb-1 hidden rounded-md bg-slate-900 px-2 py-1 text-xs whitespace-nowrap text-white shadow group-hover:block">
      ...
      <div className="ml-12 flex justify-around gap-1 pt-2 text-xs text-slate-500" aria-hidden>
        {points.map((p) => (
